...
- **UI redesign — public landing (hero) page + 2FA brand consistency.** The public
  `hero.html` landing page was reworked into a premium dark "Trust & Authority"
  enterprise hero (per the ui-ux-pro design guide): a deep navy/slate fold with a
  restrained indigo→violet aurora accent and a faint masked grid, an eyebrow status
  pill, a dual CTA, a trust-signal row, and a frosted product-window mock of the
  document-status card — with the header and the closing CTA band darkened to
  bookend it. `static/css/hero.css` was rewritten onto the `--nx-*` tokens; the
  mid-page sections (process journey, feature cards with gradient icon chips,
  "getting started" medallions, FAQ) sit on a light brand tint between the dark
  bands. Grounded in navy rather than a full-bleed purple wash (an explicit
