...
- **UI preferences:** per-user appearance settings (theme incl. system, accent color, motion, entrance, density, sidebar pin) live in `dbo.Users.ui_prefs` (JSON, allowlisted in `nx_lib/ui_prefs.py`), hydrate into `session['ui_prefs']` once per session, and are applied pre-paint by the script in `templates/_header.html` `<head>` (`window.nxSetUiPref` persists changes via `POST /profile/ui_prefs`). Profile page "Appearance" section is the UI; accent/density/motion CSS lives in `static/css/nexora-ui.css` (`html[data-accent]`, `html.nx-compact`, `html.nx-motion-reduced`, `html[data-entrance]`).
