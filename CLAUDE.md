...
- **Routing:** Routes live in `nx_lib/views/` (`auth`, `admin`, `dashboard`, `workitems`, `core`, `generali`, `profile`, `reporting`, `api_external`). `views/invoices.py` is archived (#177) — kept in tree but deliberately never registered, so its routes 404. Templates are flat under `templates/` with a few subfolders: `admin/` (admin pages + `modals/`), `handlers/` (403/404/500), `js/` (per-page JS as Jinja partials, included by the matching page template), `archive/` + `js/archive/` (retired pages, not routed), `jd/`, `nexora_logo/`. Page template `foo.html` typically pairs with `templates/js/_foo_js.html`.
- **Static JS partials (#191):** the three biggest partials (`_reporting_simple_js.html`, `_reporting_dashboard_js.html`, `_header_js.html`) are now *shims*: the inline `<script nonce>` holds only Jinja-rendered data (an `I18N`/`NX_*` object literal with the translated strings, `url_for()` endpoints, perm flags, the Ctrl+K command list), and the behaviour lives in `static/js/<name>.js`, loaded with `<script src="{{ static_v('js/<name>.js') }}">`. `static_v()` (a Jinja global in `nx_lib/__init__.py`) appends an mtime `?v=` so a deploy busts the browser cache. **Babel is unaffected** — `babel.cfg` still extracts from `templates/**.html`, so a translated string must stay in the shim and be read from `window` in the `.js`. A `.js` file has no `url_for()`: build URLs with the `API_PREFIX` idiom. `tests/unit/test_template_url_prefix.py` lints both rules. When a partial gets big enough to hurt, split it the same way; the remaining partials stay inline.
