...
**Multi-source workitems (MS02)** live in `nx_lib/workitem_sources.py` (adapters + probe-then-cache routing) and `nx_lib/clients.py` (client registry). Statistics route via `dbo.Statconfig.ClientCode`, doc-field search via `dbo.SearchConfig.ClientCode`; for MS02 both read wide **columnar** statistik tables (e.g. `public."DossierStatistik"`), *not* an EAV index. **Engines, migrations `0023`–`0033`, resolvers, permission gates, gotchas: `docs/design/ms02-multisource.md`.**
