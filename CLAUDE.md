...
- **Routing** — routes in `nx_lib/views/`, either a single module or a package (`generali/`, `admin/`) of submodules re-exported from `__init__.py`; templates flat under `templates/` plus `admin/`, `handlers/`, `js/`, `js/archive/`, `jd/`, `nexora_logo/`. Page `foo.html` pairs with `templates/js/_foo_js.html`.
