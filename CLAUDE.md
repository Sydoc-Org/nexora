...
- **UI preferences:** per-user appearance settings (theme incl. system, accent color incl. custom hex, motion, entrance, density, font scale, corner radius, contrast, table stripes, page background, sidebar pin) live in `dbo.Users.ui_prefs` (JSON, allowlisted in `nx_lib/ui_prefs.py`), hydrate into `session['ui_prefs']` once per session, and are applied pre-paint by the script in `templates/_header.html` `<head>` (`window.nxSetUiPref` persists changes via `POST /profile/ui_prefs`). The `/appearance` page (linked from profile, profile dropdown, Ctrl+K) is the UI, with a live preview canvas; CSS knobs live in `static/css/nexora-ui.css` (`html[data-accent]`, `html[data-entrance]`, `html[data-fontscale]`, `html[data-radius]`, `html[data-bg]`, `html.nx-compact`, `html.nx-motion-reduced`, `html.nx-contrast`, `html.nx-stripes`).
