...
**Branching model.** Work happens on a **short-lived topic branch cut from `main`**, named `<type>/<slug>` with the issue number in the slug (`fix/253-collab-rules`); `<type>` is a Conventional-Commit type. `v<x.y.z.n>` branches are **legacy** — only ones already in flight still push, never cut a new one. The pre-push hook refuses any push whose HEAD is `main` (GitHub cannot: private repo on the Free plan has no branch protection). A release is a **tag** on `main`, not a branch. Before writing a SQL migration, claim its `NNNN` in the issue — two sessions picking the same number is the standard collision, and migrations are immutable once applied. Never hand-merge generated files (`sql/<Db>/*.sql`, `translations/*.po`) — regenerate them. **Full rules, conflict hotspots and the release recipe: `CONTRIBUTING.md` → "Working in parallel" and "Releases".**
