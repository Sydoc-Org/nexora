...
*.edit.org could read every org's records by omitting or spoofing
...
def _scope(app, monkeypatch, perms, session_org, session_uid, requested_org, org_users):
    ...
    with app.test_request_context():
        ...
        return gv._generali_scope_where(
            "tenant.generali.reporting", "ReportByUserID", requested_org
        )


def test_transorg_no_request_org_is_unconstrained(app, monkeypatch):
    clauses, params = _scope(
        app, monkeypatch, {"tenant.generali.reporting.edit.all"}, "A", "u1", "", {}
    )
    assert clauses == []
    assert params == []
...
def test_transorg_honours_requested_org(app, monkeypatch):
    clauses, params = _scope(
        app,
        monkeypatch,
        {"tenant.generali.reporting.edit.all"},
        "A",
        "u1",
        "B",
        {"B": [10, 11]},
...
def test_org_only_ignores_requested_org_and_clamps_to_session_org(app, monkeypatch):
    """The core fix: a caller holding only *.edit.org who requests a
    RIVAL org must be clamped to their OWN session org, never the requested
    one."""
    clauses, params = _scope(
        app,
        monkeypatch,
        {"tenant.generali.reporting.edit.org"},
        "A",
        "u1",
...
def test_empty_org_fails_closed(app, monkeypatch):
    ...
    clauses, params = _scope(
        app, monkeypatch, {"tenant.generali.reporting.edit.org"}, "A", "u1", "", {"A": []}
    )
    assert clauses == ["1=0"]
    assert params == []
