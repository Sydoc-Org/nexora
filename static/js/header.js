...
(function () {
    ...
    function buildItem(action, index) {
        ...
        li.className =
            "cmdk-item" + (index === selectedIdx ? " is-selected" : "");
        li.setAttribute("role", "option");
        li.dataset.idx = String(index);

        const icon = document.createElement("i");
        icon.className =
            "fas " + (action.icon || "fa-arrow-right") + " cmdk-item-icon";
        li.appendChild(icon);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (dmToggle) {
        dmToggle.addEventListener("click", () => {
            ...
            if (window.nxSetUiPref)
                window.nxSetUiPref("theme", nowDark ? "dark" : "light");
            else
    ...
    function applyPinned(isPinned) {
        ...
        if (pinIcon)
            pinIcon.className = isPinned
                ? "fas fa-thumbtack sidebar-nav-icon sidebar-pin-active"
                : "fas fa-thumbtack sidebar-nav-icon";
        if (pinLabel)
            pinLabel.textContent = isPinned
                ? window.NX_HEADER.i18n.unpinSidebar
                : window.NX_HEADER.i18n.keepSidebarOpen;
    }
    ...
    if (pinToggle) {
        pinToggle.addEventListener("click", () => {
            ...
            if (window.nxSetUiPref)
                window.nxSetUiPref("sidebar", nowPinned ? "pinned" : "auto");
            else
                localStorage.setItem(
                    "nexora-sidebar-pinned",
                    nowPinned ? "true" : "false",
                );
    ...
    (function () {
        ...
        function spawn(field) {
            for (let i = 0; i < COUNT; i++) {
                ...
                dot.style.setProperty(
                    "--ff-glow-dur",
                    rand(3, 6).toFixed(1) + "s",
                );
                dot.style.setProperty(
                    "--ff-drift-dur",
                    rand(7, 13).toFixed(1) + "s",
                );
                dot.style.setProperty(
                    "--ff-delay",
                    rand(0, 8).toFixed(1) + "s",
                );
        ...
        function syncFireflies(bg) {
            ...
            else if (!wantFireflies && field.childElementCount)
                field.innerHTML = "";
        }
...
(function () {
    function headerTextFor(th) {
        ...
        const t = (th.innerText || th.textContent || "")
            .trim()
            .replace(/\s+/g, " ");
    ...
    function scan(root) {
        (root.querySelectorAll ? root.querySelectorAll("table") : []).forEach(
            applyLabels,
        );
    }
...
(function () {
    ...
    function render(filter) {
        ...
        const filtered = q
            ? usernames.filter((u) => u.toLowerCase().includes(q))
            : usernames;
        list.innerHTML = "";
        if (!filtered.length) {
            const empty = document.createElement("li");
        ...
        filtered.forEach((u) => {
            ...
            li.addEventListener("click", () => {
                window.location.href =
                    API_PREFIX + "dev/login/" + encodeURIComponent(u);
            });
            list.appendChild(li);
        });
    }
    ...
    async function open() {
        ...
        setTimeout(() => input.focus(), 10);
        if (!usernames.length) {
            const r = await fetch(API_PREFIX + "dev/users", {
                credentials: "same-origin",
            });
...
document.addEventListener("DOMContentLoaded", function () {
    document
        .querySelectorAll(".sidebar-nav-group[data-nx-nav-group]")
        .forEach((group) => {
