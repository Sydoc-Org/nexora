...
keys. Four documented endpoints in v1. Code: routes in
...
`resolve_invoice_import_datetime` in
...
| 500 | `{"error": "Stats backend unavailable"}` (`/avg_processing_time`, `/undelivered`, `/invoice/import_datetime`) or `{"error": "Backlog backend unavailable"}` (`/backlog`) or `{"error": "Internal server error"}` | stats/backlog/invoice query or server failure |
