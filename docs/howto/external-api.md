...
(`compute_today_stats` / `resolve_invoice_import_datetime` in
...
| 500 | `{"error": "Stats backend unavailable"}` (`/stats/today`, `/invoice/import_datetime`) or `{"error": "Backlog backend unavailable"}` (`/backlog`) or `{"error": "Internal server error"}` | stats/backlog/invoice query or server failure |
