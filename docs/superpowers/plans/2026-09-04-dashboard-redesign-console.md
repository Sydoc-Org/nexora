...
- **Commit subjects are capped at 72 characters** (`T1 Title exceeds max length`) and must start with a conventional type (`feat fix chore refactor docs test ci perf style build revert` — `merge:` is rejected, use `chore:`). Several subjects written out below exceed the cap; shorten them and keep the meaning. Measured on Tasks 5 and the peer merge.
...
- **Run unit tests with `ENVIRONMENT=INT`; run integration tests with NO `ENVIRONMENT` set.** `tests/conftest.py` selects the TEST database itself, and an `ENVIRONMENT=INT` override points the app at NexoraDB instead of `NEXORA_TEST` — every `*_client` fixture then fails login with `AssertionError: /login failed for user@test.local: status=401`. Measured on Task 2, where it produced 78 phantom "pre-existing" errors. Correct commands:
  - unit: `PATH="/c/dev/nexora/.venv/Scripts:$PATH" ENVIRONMENT=INT python -m pytest tests/unit/... -q`
  - integration: `PATH="/c/dev/nexora/.venv/Scripts:$PATH" python -m pytest tests/integration/... -q --no-cov`
- **The test-DB reset script is `scripts/test_db_reset.py`** (not `tests/test_db_reset.py`), and it is not needed for the dashboard suites.
- **Branch / worktree:** planned in worktree `.claude/worktrees/plan-dashboard-redesign-console` on branch `plan/dashboard-redesign-console`, cut from `refactor/255-admin-nav-tenancy-labels` @ `ebbcdf07`, then **merged up to `26909d46` in `a08b07d5`** — see the peer-work note below. Execute there.
- **⚠ A parallel session owns `refactor/255-admin-nav-tenancy-labels` and is actively committing to it.** Its commit `c6df1b9f` introduced `_allowed_processes()` / `_PROCESS_PERM_PREFIX` into `nx_lib/views/dashboard.py`; this plan was written against that code while it was still *uncommitted* in the main checkout, so the worktree's base did not have it and Tasks 4-6 would have failed. The merge fixed it. **Before Task 7 and again before any merge back, re-run `git log --oneline HEAD..refactor/255-admin-nav-tenancy-labels`** — if it is non-empty, merge again rather than diverging further. The peer also owns migrations `0097` and `0098`. The main checkout has uncommitted work in `nx_lib/tenant/registry.py` and `nx_lib/views/tenant.py` — never stage those.
...
- **Do not trust a full-suite run taken while a peer session is active.** `NEXORA_TEST` is shared across every checkout on this machine. Running `pytest tests/integration` end to end (~8 min) while the peer session runs its own suite produced **174 failures and 39 errors** on 2026-09-04 — all of which passed on re-run, file by file, in both checkouts (`test_workitems_routes.py`: 108 passed in the worktree, 108 in the main checkout). Verify per-file before concluding anything is broken, and prefer targeted runs while a peer is committing.
