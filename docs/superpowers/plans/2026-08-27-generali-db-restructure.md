...
| D9 | ~~The effort cluster is **renamed in phase 2 and merged in phase 5**, not both at once.~~ **Renamed in phase 2; merge dropped in phase 5.** | The rationale was that the merge is an app refactor (~800 lines of `generali.py`). That refactor happened separately — `_crud.py` is now a descriptor factory — so the merge would save no code, and it would collapse the four `reporting.source.*.use` grants into one. See phase 5.2. |
| D10 | ~~`CategoryTranslation` folds into per-locale columns on the catalogue tables~~ **`CategoryTranslations` pivots into `CategoryTerms(NameDe, NameEn, NameFr, NameIt)`.** | The catalogue tables are *combination* tables, not `(Id, Name)` catalogues, so per-locale columns on them would duplicate each parent's translation across up to ten rows. The pivot still kills the join, the `WHERE Locale = ?` **and** the `SourceTable` discriminator. See phase 5.1. |
...
## Phase 5 — Better logic — **DONE 2026-09-10** (5.2/5.3 deliberately skipped)

- [x] **5.1** `0013_category_terms.sql` retires `dbo.CategoryTranslations`, the last place in this database storing **table names as data**. **Not the shape D10 proposed** — the plan assumed `EffortCategories`/`QualityCheckCategories` are `(Id, Name)` catalogues like `NexoraDB.dbo.FieldLabels`. They are not: they are *combination* tables (32 effort rows carry 6 distinct parents; `QualityCheckCategories` repeats `GAV Retouren` ten times), and `CategoryTranslations` translates the distinct German **terms** appearing in any of their columns, not rows. Per-locale columns on the catalogues would mean 6 new columns on one table and 9 on the other, with each parent's translation duplicated across up to ten rows — a permanent update anomaly, to remove a join over 162 rows.
  - **Done instead: pivot to `dbo.CategoryTerms(Id, NameDe, NameEn, NameFr, NameIt)`, 53 rows.** One row per term, locale as a column. Both offending columns disappear: `SourceTable` (verified to carry *no* information — zero terms appear under both source tables, so the partition is redundant, and a shared dictionary is more correct: the same German word should not get two different French words) and `Locale` (162 rows → 53). Every term has all three of en/fr/it today, so NULL now honestly means "not translated" rather than "row missing".
  - App: the two near-identical 12-line blocks in `attendance.py` and `pdqm.py` collapse to one call to `_generali_category_terms(conn, locale)` in `_scope.py`. The endpoint response shape is unchanged — the browser still gets a `{German: localised}` map.
  - Compat view `dbo.CategoryTranslations` (D8) reconstructs all five original columns, deriving `SourceTable` from the catalogues. **It is the one piece of scaffolding this release adds, so it survives 0014 and comes out in the next release.**
- [x] **5.1b** `0015_trim_category_values.sql` — a latent bug 5.1 exposed. `QualityCheckCategories` held `'aus sonstigen Quellen '` *and* `'aus sonstigen Quellen'`; someone typed the same category twice, once with a stray blank. Nothing had ever noticed because **SQL Server ignores trailing spaces in comparison** — `=`, `<>`, `GROUP BY` and `DISTINCT` all treat the two as one value, so the standard `WHERE col <> LTRIM(RTRIM(col))` audit returns nothing. Only `DATALENGTH` sees it.
  - Harmless while translations were one row per (term, locale) — the table simply carried both spellings and `dict(cursor.fetchall())` got a key for each. After the pivot, `GROUP BY` collapsed them (correctly: byte-identical translations) and the padded spelling had no key left. The lookup happens in **JavaScript, by exact string**, so five `GAV Retouren` subcategories would have silently fallen back to German.
  - 7 rows trimmed (`QualityCheckCategories` ids 6-10, `QualityCheckEntries` ids 5, 8 — the entries store the category as text, not as an FK). Verified no duplicate catalogue rows result; the unpadded twin lives under a different parent. Guard re-checks every catalogue term is reachable from `CategoryTerms` by exact bytes.
- [ ] ~~**5.2** merge the four effort tables into `EffortEntries`~~ — **skipped, deliberately.** Two independent reasons:
  1. **The payoff was already collected.** D9 justified the merge as "an app refactor (~800 lines of `generali.py`)". That refactor has since happened on its own: `nx_lib/views/generali/_crud.py` is a descriptor-driven factory and each page contributes only a `CrudTable`. Merging the tables now saves no application code — it *adds* an `EntryType` predicate to every generated statement, including the delete/edit scoping so one page cannot touch another's rows.
  2. **It would collapse a permission boundary.** `generali_attendance`, `generali_baseservices`, `generali_projects` and `generali_pdqm` are four reporting sources with four separate `reporting.source.*.use` grants, separate *only* because they name four different objects. `ReportingSources` has no predicate column, and `_quote_object()` in `nx_lib/reporting/table_query.py` splits `BaseObject` on dots and bracket-quotes each part, so a `WHERE` cannot ride along in it. The Simple-tab filters are the user's, chosen at run time and saved into the report definition — they cannot pin a source. Point all four at one table and a PDQM grant reads attendance hours by default, and a user would have to *add* a filter to see less.
  - Revisit the day `ReportingSources` grows a source-level filter. 20 rows across four tables costs nothing until then.
- [ ] ~~**5.3** per-locale names on the 13 lookups~~ — **skipped, deliberately.** Nothing translates them today: the app never queries those tables directly, it reads their German text through `v_Documents` and renders it as-is. The columns would be ~250 empty cells (`DocumentTypes` alone has 159 rows) waiting for translations nobody has written. Scaffolding for later; later can scaffold for itself. Do it the day someone supplies the translations.

## Phase 6 — Take the scaffolding down — **DONE 2026-09-10**

- [x] **6.1 / 6.2** `0014_drop_compat_views.sql` drops all 11 compat views from phases 2-3 in one catalogue-driven pass: `AdditionalServices`, `Attendance`, `BaseServices`, `CategoryTranslation`, `CSVImportLog`, `PDQMMapping`, `PDQMReport`, `ProjectManagement`, `ReportingISS`, `ReportJob` and `v_ReportJobJoinDefinitions` (6.2). The loop drops a name only if `OBJECT_ID(…, 'V')` resolves, and **throws if the name is still a base table** — if a rename had ever failed, that would be data loss rather than cleanup. A precondition block refuses to run at all unless every renamed target exists.
  - Both gates met. Q3 (external consumers) was answered "no" by the owner, and **re-verified against evidence**: INT's Query Store over 31 days (2026-08-10 → 2026-09-10, 884 distinct query texts) shows every reader of these 11 names is this repo's own pre-#318 code — `SELECT DISTINCT UserID FROM [Generali].[dbo].[Attendance]` (last 2026-09-07), the same for `BaseServices` (2026-08-31) and `reportingiss` (2026-09-07), and the old `DOC_*`-column document queries against `v_ReportJobJoinDefinitions` (last 2026-09-10 08:23, two hours before #318 merged). Nothing external, nothing after the cutover.
  - Ordering gate met: phases 1-4 are on PROD (#318 `24e87fad`, #319 `5f4de843`) and prdimpexp01 runs the matching `csvToSql.ps1`. `ReportingSources.BaseObject` points all seven Generali sources at a real table or at `v_Documents` — checked, no source names a compat view.
  - `dbo.v_Documents` stays: it is the app's read path in all 12 document queries and the base object of `generali_documents`, not scaffolding.
  - The phase-4 text columns (`AmountText`, `QuantityText`, `PendingText`, `VoucherDateText`) **stay**, and the plan's original 6.1 wording asking to drop them is obsolete: 4.1 shipped the typed columns as *computed* expressions over those strings, so the strings are the data. Dropping them would drop `Amount`/`Quantity`/`VoucherDate`/`IsPending` with them.
- [x] **6.3** Re-dumped (`sql/sync-from-db.py`) and wrote **`docs/design/generali-tenant-db.md`** — the naming rulebook, the shape of each table group, the gotchas that cost time in this restructure (filtered index as a rename dependency, `QUOTED_IDENTIFIER` under sqlcmd, catalogue-driven constraint renaming, the trailing-space trap), and a "decisions that look wrong" section recording why 5.2, 5.3 and 4.4 were *not* done, so the next person does not re-propose them from scratch.

**Left for the next release** (small, one migration): drop the `dbo.CategoryTranslations` compat view added by 0013.
