...
- [ ] **2.4** `NexoraDB` migration (next free number — `0127` is taken, check `--dry-run`): update `dbo.ReportingSources.BaseObject` for **both** Generali rows — `generali_pdqm` (`dbo.PDQMReport` → `dbo.QualityCheckEntries`) and `generali_documents` (`dbo.v_ReportJobJoinDefinitions` → `dbo.v_Documents`, phase 3). Q1 is answered; neither is mis-registered.
