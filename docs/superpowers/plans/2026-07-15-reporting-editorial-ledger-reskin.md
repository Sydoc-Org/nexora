...
- **In-flight overlap (verified 2026-07-15):** a parallel session is editing `templates/js/_reporting_metrics_js.html`, `templates/reporting_metrics.html`, `tests/e2e/test_reporting_metrics.py`, `tests/unit/test_reporting_query.py` in the main tree. This plan touches **none** of those (admin pages are out of scope) — but re-verify anchors at execution if more of its commits land. The dormant 2026-06-12 AI-clarifications plan touches the same JS partials; it is NOT executed — do not assume its chips exist.
...
- [ ] e2e: `…python -m pytest tests/e2e/test_reporting_simple.py -q`. Screenshot `var/screenshots/ledger-task7.png`.
- [ ] Commit:
...
tracked caption style; the chart block gets the signature 2px ink
top rule on both panes.
