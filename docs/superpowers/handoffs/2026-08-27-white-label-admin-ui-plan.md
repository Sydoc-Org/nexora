...
**Date:** 2026-08-27 · **Branch:** `v3.2.3.1` (main checkout, no worktree) · **1 commit this
session; 26 commits ahead of `origin/v3.2.3.1`, unpushed** · commit-only (owner pushes) · tree
carries CRLF-only dump dirt that is not this session's — see Gotchas.
...
1. `/execute-plan` on `docs/superpowers/plans/2026-08-27-white-label-admin-ui.md` — start at
   **Task 1** (migration `0079`). **Re-check migration numbering first**: `ls
   ...
   INT --dry-run` must be clean — it is **not** clean right now, see Gotchas #1.
...
3. Owner: reconcile the `0076` migration checksum (Gotchas #1) — it blocks every SQL-touching commit
   on this branch, not just this workstream.
4. Owner: push the 26 unpushed commits on `v3.2.3.1`.
5. Optional: file the phase-4 GitHub issue linking the spec + plan (the #98 handoff asked for one;
   the owner chose spec-first instead this session).

## Gotchas & notes

1. **`0076_seed_column_types.sql` was edited after being applied to INT.** `python
   scripts/db-migrate.py --env INT --dry-run` errors with *"1 migration(s) edited after being
   applied … Migrations are immutable"*, and the `sql-migrate-int` pre-commit hook **blocks any
   commit** because of it. It arrived with the `feat/docfield-restructure` merge (`d0415388`) — a
   peer's work, deliberately not touched here. This session's docs-only commit got through with the
   sanctioned `SQL_SYNC_SKIP=1 git commit` hatch (never `--no-verify`). **Fix it before Task 1**, or
   every migration task in the plan will trip on it.
...
python scripts/db-migrate.py --env INT --dry-run    # CURRENTLY FAILS - see Gotchas #1
...
Nothing was implemented this session, so no suite changed state. The `db-migrate --dry-run` failure
above is **pre-existing** and inherited from the merge, not caused by this work.
