...
The dashboard's Statistics-DB-backed widgets (`processed_over_time`, `kpi_stats` processed/imported counts, and the other chart endpoints) are driven by `Statconfig` (NexoraDB): `ProcessName → (TableName, ExportColumn, ImportColumn, additionalCondition)`, aggregated against `[{DB_STATISTICS}].{TableName}` on `engine_statistics_db`. MS02's processing-event data lives in **`public.batchtracking`** in a **separate Postgres database `Praesidialdepartement_BS`** (its own DB — not the MS02 workitems DB, and not `t_documentindexes`): `datuminexport` = processed/export timestamp, `datumimportiert` = import timestamp, with a `workitemid` column available. **MS02 stats are not split per process** — the dashboard aggregates the whole `batchtracking` table for MS02.

Design:
- **Second engine.** A Postgres connection is bound to one database, and `Praesidialdepartement_BS` is separate from the MS02 workitems DB — so add `engine_ms02_stats_pg` → `Praesidialdepartement_BS` (new `MS02_STATS_DB_*` env, likely the same Azure host/login as `MS02_DB_*` with `dbname=Praesidialdepartement_BS`; same graceful-degrade). Each `ClientConfig` gains a `stats_engine` (default → `engine_statistics_db`; ms02 → `engine_ms02_stats_pg`) + `stats_dialect`.
- **Gating via `Statconfig.ClientCode`** (migration 0024, default `'default'`). Seed one MS02 row per MS02 process — `TableName='public.batchtracking'`, `ExportColumn='datuminexport'`, `ImportColumn='datumimportiert'`, `ClientCode='ms02'` — so the existing `WHERE ProcessName IN (target_processes)` naturally gates MS02 in/out by the user's permissions.
- **Aggregate once (no per-process).** The dashboard groups configs by `ClientCode`; for the `ms02` group it **dedupes to a single `batchtracking` query** (all MS02 rows point at the same table with no per-process filter — running per-row would multi-count) on that client's `stats_engine`, Postgres dialect, and sums the result into the default per-process totals. The central seam is the `Statconfig`→`[(engine, sql, params)]` builder (≈`dashboard.py:1186–1201`).
- **PG dialect twins:** `CAST(col AS DATE)`→`col::date`, `DATEADD(day,-14,GETDATE())`→`CURRENT_DATE - 14`, `GETDATE()`→`NOW()`, drop the `[DB_STATISTICS].` prefix, `?`→`%s`. Same graceful-degrade as the list: a down MS02 stats DB contributes nothing; default widgets still render.
